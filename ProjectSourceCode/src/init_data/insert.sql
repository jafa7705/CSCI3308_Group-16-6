...
BEGIN
    ...
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Alice_Wonder', hashedPassword, 'alice@example.com', false, 'Painter based in NYC', 'http://aliceart.com', 'New York','alicePic.jpg');

    hashedPassword := crypt('hashedpassword2', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location,profile_image) VALUES ('Bob_Marley', hashedPassword, 'bob@example.com', true, 'Art collector', NULL, 'Los Angeles', 'bobPic.jpg');

    hashedPassword := crypt('hashedpassword3', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Carla_Gugino', hashedPassword, 'carla@example.com', false, 'Digital artist', 'http://carladraws.io', 'Austin', 'carlaPic.jpg');

    hashedPassword := crypt('hashedpassword4', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Dan_Stan', hashedPassword, 'dan@example.com', false, NULL, NULL, 'Chicago', 'defaultProfilePic.png');

    hashedPassword := crypt('hashedpassword5', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Eva_Steller', hashedPassword, 'eva@example.com', true, NULL, NULL, 'Miami', 'defaultProfilePic.png');

    hashedPassword := crypt('hashedpassword6', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Frank_Devito', hashedPassword, 'frank@example.com', false, 'Sculptor', NULL, 'Denver', 'defaultProfilePic.png');

    hashedPassword := crypt('hashedpassword7', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Grace_Kennedy', hashedPassword, 'grace@example.com', false, NULL, NULL, 'Seattle', 'defaultProfilePic.png');

    hashedPassword := crypt('hashedpassword8', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Henry_Haden', hashedPassword, 'henry@example.com', false, 'Animator', NULL, 'Boston', 'defaultProfilePic.png');

    hashedPassword := crypt('hashedpassword9', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Demo_Artist', hashedPassword, 'irene@example.com', false, 'I’m a ceramic artist crafting functional, earthy pottery inspired by nature and simple moments. Each piece is handmade with care — designed to be used, loved, and part of your daily rituals.', NULL, 'San Diego', 'defaultProfilePic.png');

    hashedPassword := crypt('hashedpassword10', gen_salt('bf'));
    INSERT INTO users (username, password, email, isClient, bio, website, location, profile_image) VALUES ('Demo_Employer', hashedPassword, 'jack@example.com', true, 'I’m the owner and curator of Lumen Gallery, a space dedicated to showcasing emerging and established artists across a range of mediums. With a passion for storytelling through visual art, I strive to create exhibitions that inspire conversation, connection, and curiosity.', NULL, 'San Francisco', 'defaultProfilePic.png');
END $$;
